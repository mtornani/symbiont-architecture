"""
benchmark_setpoint_shift.py  (v1 — generalizzazione: shock di setpoint)
========================================================================
Test di generalizzazione: ĝ ripara anche uno shock di SETPOINT?

CONTESTO — non riaprire i benchmark chiusi:
  v4 (chiuso): legge fissa EHD fragile su gain×4 → 10/30 mai_recuperato.
  Adattivo (chiuso): ĝ online ripara la fragilità su gain×4 → 0/30.
  Questo test chiede se ĝ generalizza. La risposta potrebbe essere NO —
  il risultato negativo è altrettanto informativo.

PUNTO DI INNESTO DELLA PERTURBAZIONE:
  In Env1D, un solo parametro cambia rispetto ai test precedenti:
  il SETPOINT salta da SETPOINT_PRE a SETPOINT_POST a t=SHIFT_T.
  Il GUADAGNO rimane costante a GAIN=1.0 per tutta la durata.
  Quindi ĝ (che stima il guadagno) rimane corretto e non ha nulla
  da ristimare — questa perturbazione è per costruzione INVISIBILE al
  meccanismo di ĝ. È il punto esatto del test.

=============================================================================
PREREG — Pre-registration (fissata prima di eseguire; NON modificare dopo)
=============================================================================

IPOTESI DI GENERALIZZAZIONE:
  ĝ, che ha riparato la fragilità su gain×4, ripara la fragilità anche
  su uno shock di setpoint (perturbazione di tipo diverso, che ĝ non è
  progettato per catturare).

CONDIZIONE A — Affidabilità generalizzata:
  H1A: mai_recuperato_adaptive < mai_recuperato_fixed su shock di setpoint
  FALSIFICATA se: mai_recuperato_adaptive >= mai_recuperato_fixed
  Nota: se entrambi = 0, H1A è falsificata (ĝ non ha aiutato).
  Nota: se entrambi > 0 e uguali, H1A è falsificata (ĝ non ha aiutato).

CONDIZIONE B — Precisione preservata:
  H1B: pre_baseline_adaptive < pre_baseline_q   [Welch one-tailed, p < 0.05]
  FALSIFICATA se: p >= 0.05

PERCHÉ UN RISULTATO NEGATIVO È INFORMATIVO:
  Se H1A è falsificata: ĝ ripara solo le perturbazioni che STIMA (guadagno),
  non generalizza a perturbazioni di natura diversa (setpoint). Questo
  definisce il confine del meccanismo: ĝ è uno stimatore specializzato,
  non un correttore universale.

MECCANISMO DI INVISIBILITÀ DI ĝ:
  Il modello interno di ĝ traccia: pred_err = error_t+1 - (error_t + ĝ * a_t)
  Con gain costante = 1.0, pred_err ≈ (1 - ĝ) * a_t + noise → ĝ resta ≈ 1.0.
  Quando il setpoint salta: al primo step post-shift appare un termine
  spurio -delta_setpoint nel pred_err, ma è un artefatto di una sola
  iterazione — poi la stima di ĝ si stabilizza di nuovo vicino a 1.0.
  L'agente adattivo si comporta quindi come la legge fissa su questo shock.

STESSO AMBIENTE dei test precedenti, eccetto la perturbazione:
  30 seed. Stesse metriche. Stesso gate plateau. Stessi self-check scettici.

ONESTÀ:
  Un risultato negativo è atteso e informativo. Nessuno spin.
=============================================================================

Run:
    cd symbiont-architecture
    python benchmark_setpoint_shift.py
"""
...
# ---------------------------------------------------------------------------
# Costanti — identiche ai test precedenti, tranne SETPOINT_POST e GAIN costante
# ---------------------------------------------------------------------------
T_PRE             = 1000
T_POST            = 200
...
GAIN              = 1.0            # costante per tutta la durata — NON cambia
SETPOINT_PRE      = 0.0
SETPOINT_POST     = 2.0            # il bersaglio salta: invisibile a ĝ
...
# Parametri agente adattivo — identici al test gain×4
LR_GAIN           = 0.05
GAIN_HAT_MIN      = 0.5
GAIN_HAT_MAX      = 10.0
GAIN_HAT_INIT     = GAIN           # prior equo = guadagno vero (costante)

# ---------------------------------------------------------------------------
# Ambiente 1D — SOLO il setpoint cambia a SHIFT_T; il guadagno NON cambia
# ---------------------------------------------------------------------------
class Env1D:
    """
    x_{t+1} = x_t + GAIN * clip(action,-1,1) + noise
    setpoint = SETPOINT_PRE per t < SHIFT_T, poi SETPOINT_POST.
    Nessun agente viene notificato del salto del setpoint.
    """
...
# ---------------------------------------------------------------------------
# Agente omeostatico FISSO — identico ai test precedenti
# ---------------------------------------------------------------------------
class SymbiontAgent:
    N_NEURONS = 4
    N_INPUTS  = 8
...
# ---------------------------------------------------------------------------
# Agente omeostatico ADATTIVO — identico al test gain×4
# ---------------------------------------------------------------------------
class AdaptiveSymbiontAgent:
    """
    EHD con stima online del guadagno (identico al test gain×4).

    Il modello interno traccia error-space:
      pred_err = error_t+1 - (error_t + ĝ * action_t)
              = (gain - ĝ) * action_t + noise   [quando setpoint è costante]

    Quando il setpoint salta, al primo step post-shift appare un termine
    spurio -delta_setpoint: ĝ riceve un piccolo impulso negativo
    (attenuato dall'azione che era piccola pre-shift, ≈0), poi si ri-stabilizza.
    Successivamente ĝ ≈ 1.0 (gain vero) e l'agente si comporta come il fisso.
    """
    N_NEURONS = 4
    N_INPUTS  = 8
    ...
    def act(self, error: float) -> float:
        x_current = error  # traccia error-space; pred_err = (gain-ĝ)*a + noise a setpoint costante
...
# ---------------------------------------------------------------------------
# Q-learning — identico ai test precedenti
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Episodio — tre ambienti identici (stesso seed → stessa traiettoria rumore)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Metriche — identiche ai test precedenti
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Welch t one-tailed — identico ai test precedenti
# ---------------------------------------------------------------------------
...
def run_benchmark():
    ...
    print(f"Benchmark setpoint shift: {N_SEEDS} seed, T={T_STEPS} (shift a t={SHIFT_T})")
    print(f"  Setpoint: {SETPOINT_PRE} → {SETPOINT_POST}  |  Gain costante: {GAIN}")
    print(f"  Recovery = baseline_propria + {EPSILON}")
...
def print_results(all_f: List[Metrics], all_a: List[Metrics], all_q: List[Metrics]) -> None:
    ...
    print("=" * 82)
    print(f"  * plateau_ratio ≥ {PLATEAU_MIN_RATIO} → convergito  [C] shock divergenti → velocità va letta con cautela")
    ...
    # BLOCCO 1 — PRECISIONE
    t_af, p_af = welch_t_one_tailed(prec_a, prec_f)
    t_aq, p_aq = welch_t_one_tailed(prec_a, prec_q)
    t_fq, p_fq = welch_t_one_tailed(prec_f, prec_q)
    ...
    if np.std(rt_f) > np.mean(rt_f) + 1e-9:
        n_never = nr_f
        print(f"\n  [!] Fisso: std({np.std(rt_f):.1f}) > media({np.mean(rt_f):.1f}) — distribuzione degenere"
              f"  mai_recuperato={n_never}/{N_SEEDS}")
    if np.std(rt_a) > np.mean(rt_a) + 1e-9:
        print(f"  [!] Adattivo: std({np.std(rt_a):.1f}) > media({np.mean(rt_a):.1f}) — distribuzione degenere"
              f"  mai_recuperato={nr_a}/{N_SEEDS}")

    # BLOCCO 3 — VELOCITÀ
    shock_note_fa = ("comparabili ✓"
                     if abs(np.mean(shk_f) - np.mean(shk_a)) < 1.0
                     else "DIVERGONO — confronto fisso vs adattivo non equo")
    shock_note_aq = ("comparabili ✓"
                     if abs(np.mean(shk_a) - np.mean(shk_q)) < 1.0
                     else "DIVERGONO — velocità adattivo vs Q va letta con cautela")
    ...
    print(f"  Recovery Fisso:    media={np.mean(rt_f):.1f}  mediana={np.median(rt_f):.1f}")
    print(f"  Welch (Adattivo vs Q) t={t_aq3:.3f}  H1: Adattivo < Q (Adattivo più veloce)")
    print(f"  ESITO velocità: {_esito_label(p_aq3, np.mean(rt_a), np.mean(rt_q), 'ADT PIÙ VELOCE', 'Q PIÙ VELOCE')}")

    # VERDETTO PREREG
    print(f"\n{'═'*82}")
    print("VERDETTO PREREG — TEST DI GENERALIZZAZIONE")
    print(f"{'═'*82}")

    print(f"\n  CONDIZIONE A — Affidabilità (H1A: mai_recuperato_adaptive < fisso):")
    print(f"    Fisso:    {nr_f}/{N_SEEDS}")
    print(f"    Adattivo: {nr_a}/{N_SEEDS}")
    print(f"    Q:        {nr_q}/{N_SEEDS}")
    if nr_a < nr_f:
        esito_a = f"H1A NON FALSIFICATA — ĝ migliora affidabilità anche su setpoint shift ({nr_a} < {nr_f})"
    elif nr_a == 0 and nr_f == 0:
        esito_a = (f"H1A FALSIFICATA — entrambi 0/30: ĝ irrilevante su questo shock "
                   f"(né aiuta né peggiora; il fisso già recuperava)")
    else:
        esito_a = (f"H1A FALSIFICATA — ĝ NON ripara la fragilità su setpoint shift "
                   f"(mai_recuperato_adaptive={nr_a} >= fisso={nr_f})")
    ...
    if a_ok and b_ok:
        verd = "IPOTESI NON FALSIFICATA — ĝ generalizza anche allo shock di setpoint"
    elif not a_ok and b_ok:
        if nr_a == 0 and nr_f == 0:
            verd = ("IPOTESI FALSIFICATA per A — ĝ irrilevante su setpoint shift: "
                    "il fisso già recuperava (0/30), ĝ non aggiunge nulla. "
                    "Precisione EHD preservata. Confine del meccanismo: ĝ ripara solo perturbazioni di guadagno.")
        else:
            verd = ("IPOTESI FALSIFICATA per A — ĝ non ripara la fragilità su setpoint shift. "
                    "Precisione preservata. Confine del meccanismo confermato.")
    elif a_ok and not b_ok:
        verd = "IPOTESI FALSIFICATA per B — fragilità ridotta ma precisione non dimostrata"
    else:
...
def plot_results(
    ...
    all_a: List[Metrics],
    all_q: List[Metrics],
    out_path: str = "benchmark_setpoint_shift_output.png",
) -> None:
    ...
    fig.suptitle(
        f"Benchmark Setpoint Shift — setpoint {SETPOINT_PRE}→{SETPOINT_POST} a t={SHIFT_T}"
    ...
    ax.axvline(SHIFT_T, linestyle=":", color="black", linewidth=1.2,
               label=f"shift setpoint {SETPOINT_PRE}→{SETPOINT_POST}")
    ...
    # setpoint line: pre/post
    sp_line = np.where(steps < SHIFT_T, SETPOINT_PRE, SETPOINT_POST)
    ax.plot(steps, sp_line, linestyle="--", color="gray", linewidth=0.9, label="setpoint")
    ...
    # (1,1) ĝ nel tempo — deve restare ≈ 1.0 (gain non cambia)
    ax = axes[1, 1]
    ...
    ax.axhline(GAIN, linestyle="--", color="gray", linewidth=1.0, label=f"gain vero = {GAIN} (costante)")
    ax.axvline(SHIFT_T, linestyle=":", color="black", linewidth=1.2,
               label=f"shift setpoint {SETPOINT_PRE}→{SETPOINT_POST}")
    ax.axvspan(SHIFT_T, T_STEPS, alpha=0.06, color="orange")
    ax.set_xlabel("Step")
    ax.set_ylabel("ĝ (gain stimato)")
    ax.set_title("ĝ durante setpoint shift (atteso: ≈ 1.0 costante)")
    ax.legend(fontsize=8)
